...
mmMountain.add("(min-width: 1024px)", ()=>{
    ...
    infoImg.forEach(img =>{
        ...
        .from(img, {scale:0.9,duration:0.6,ease:"expo.out"})
        ScrollTrigger.observe({
            target:img,
            type:"pointer",
            onHover: () => imgTl.play(),
...
document.querySelectorAll(".mountain_hero-text>h4").forEach((btn, index) => {
    btn.addEventListener("click", () => {
      gsap.to(window, {duration: 1, scrollTo:{y:mountInfo[index], offsetY:120},ease:"back.out(1)"});
    });
  });

/* const mountTitles = document.querySelectorAll(".mountain_hero-text>h4");

mountTitles.forEach((title,index) => {

    title.addEventListener("click", ()=>{
        mountTitleTl.play();
        console.log("img")
    })
const mountTitleTl = gsap.timeline({paused:true})
.to(window, {duration : 1, scrollTo: {y: mountInfo[index], offsetY:110, autoKill:false}})
});
 */
